#!/usr/bin/env python
"""Per-training-seed blind and resolved coverage of the two seismic priors.

For each prior and each training seed: draw a fresh sample set, put it on the common data-fixed
amplitude scale, project onto the operator's blind and resolved bases, and score how often the
held-out truths fall inside the samples' central band. Three seeds per prior is what turns a single
number into an error bar over training randomness.

The statistic is SPREAD and central coverage, never blind-band energy. The bridge to practice
is that on the blind subspace the likelihood is flat, so the posterior's blind
marginal equals the prior's -- the prior's blind coverage IS the report.
...
bases. One GPU, four to six hours for three seeds per prior.
"""
...
def main() -> None:
    ...
    ap.add_argument("--n-draw", type=int, default=384, help="prior samples per prior per seed")
    ...
    ap.add_argument("--seeds", type=int, default=3, help="training seeds per prior")
    ...
    print(f"[calib] dev={dev} blind={sub.r_blind} resolved={sub.r_resolved} | "
          f"samples={args.n_draw} steps={args.steps} n_eval={args.n_eval} seeds={args.seeds}",
          flush=True)
