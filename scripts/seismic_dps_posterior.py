...
The paper's read: for one archived survey, draw a posterior ensemble under each prior by guiding
...
Writes results/seismic_dps_recon.npz: the full posterior ensemble per prior, its mean and pointwise
...
def main() -> None:
    ...
    for tag in ("oracle", "curated"):
        ...
        torch.manual_seed(0)
        np.random.seed(0)                                # same init and shot order for both priors
