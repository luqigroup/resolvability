...
only the projection does. So each (prior, training seed) batch is generated ONCE, with the same fixed
...
``results/seismic_cutoff_sweep.npz``. With ``--from-gallery`` the cached unconditional samples are
...
def main():
    ...
    ap.add_argument("--fracs", type=float, nargs="+", default=[0.003, 0.01, 0.03, 0.1],
                    help="rel_floor_frac values; 0.01 is the cutoff used here")
    ap.add_argument("--M", type=int, default=384, help="prior samples per prior per training seed")
    ...
    ap.add_argument("--from-gallery", action="store_true",
                    help="score cached prior samples on CPU instead of regenerating on GPU")
    ...
    for tag in ("oracle", "curated"):
        if args.from_gallery:
            # Cached unconditional samples, one checkpoint per prior. Enough for the cutoff question:
            ...
            # paper's per-seed read, and not reported as one.
        ...
        for S, si in batches:
            ...
            print(f"  [{tag} batch {si}] {len(S)} samples  {time.time()-t0:.0f}s", flush=True)
            del S

    print("\n  blind coverage against nominal, by cutoff (mean over training seeds)")
    print("  %-9s %-9s %-7s %s" % ("prior", "frac", "blind", "  ".join("%-6.2f" % L for L in LEVELS)))
    for tag in ("oracle", "curated"):
        for f in args.fracs:
