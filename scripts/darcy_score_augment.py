#!/usr/bin/env python
"""Score the de-freezing repair: what joint curation moves, and what it leaves alone.

Reports the archive-to-truth spread ratio on the three direction sets the corollary distinguishes --
the directions the boreholes resolve, the blind directions they do not, and the resolved subspace --
for the sensors-only archive and the jointly-curated one.
...
Read the numbers carefully, and write them carefully:
  ...
  * The control does MOVE. It is not a statistical null, and saying so would be false. What makes
    ...
    controls run by ``scripts/darcy_augment_controls.py``: an information-free placebo measurement
    moves it MORE, and it stays flat as the real boreholes' strength is swept. Leakage would scale
    with the measurement's strength; this does not.

Reads the groundwater dataset, ``results/darcy_borehole.npz``, and
...
def main():
    ...
    print("  %-22s %-13s %-13s" % ("set", "sensors-only", "+boreholes"))
    for name, V in sets:
        ...
        if args.boot:
            ...
            line += "  [%.4f, %.4f]" % (lo, hi)
        print(line)
    print("\nper-direction de-frozen (+boreholes): %s"
    ...
    # eigenvalues as the boreholes have dimensions, and their top eigenspace should be the predicted one.
    B = ch["blind_exact"]
