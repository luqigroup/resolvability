#!/usr/bin/env python
"""Unconditional samples from both seismic priors, on a common seed.

On the operator's blind subspace the likelihood is flat, so a posterior's blind marginal
IS the prior's. These unconditional samples are therefore the blind report, and they are
...
def main() -> None:
    ...
    ap.add_argument("--n", type=int, default=160, help="samples per prior")
    ap.add_argument("--steps", type=int, default=50, help="DDIM reverse steps")
    ap.add_argument("--chunk", type=int, default=16)
    ap.add_argument("--seed-index", type=int, default=0, help="which training seed of each prior")
    args = ap.parse_args()
    ...
    for tag in ("oracle", "curated"):
        ...
        torch.manual_seed(0)                       # same sampling seed for both priors
