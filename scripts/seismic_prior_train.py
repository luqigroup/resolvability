...
Which prior is trained is set by which images it sees, and nothing else about the two runs differs:

    --prior oracle   trains on the broadband reflectivity  (data key ``broadband_dm``)
    --prior curated  trains on the legacy migration archive (data key ``lsrtm5``)
...
data/checkpoints/seismic_prior_<prior>_seed<seed>.pth with the weights, the normalization statistics
and the loss history. One GPU, about four hours per seed.

    python scripts/seismic_prior_train.py --prior oracle --seed 0
"""
...
def main() -> None:
    ...
    ap.add_argument("--prior", choices=["oracle", "curated"], required=True)
    ...
    print(f"[{args.prior} seed {args.seed}] train {tuple(Xtr.shape)} | per-pixel std in "
    ...
    out = os.path.join(REPO, CKPT.format(tag=args.prior, seed=args.seed))
