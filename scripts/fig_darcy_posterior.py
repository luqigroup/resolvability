#!/usr/bin/env python
"""Groundwater single-observation posterior figure: the oracle's uncertainty against the curated one.
...
Both priors reconstruct the observation plausibly, but the curated posterior's uncertainty collapses
...
def plot_title(ax, s, color="k"):
    ...
    ax.set_title(s, fontsize=9.5, fontweight="bold", color=color, pad=4.0, loc="left")


def label(ax, s, color="k"):
    """Inset boxed label, for the image panels: it sits on flat field background."""
    ax.text(0.04, 0.955, s, transform=ax.transAxes, fontsize=9.5, fontweight="bold",
...
def main():
    ...
    def hcbar(a, im, lab, ticks=None):
        ...
        if ticks is not None:
            cb.set_ticks(ticks)
        cb.ax.tick_params(labelsize=8, length=2.2, pad=1.5)
        cb.outline.set_linewidth(0.5); cb.set_label(lab, fontsize=9.5, labelpad=2.0)
    ...
    def vcbar(a, im, lab, ticks=None):
        ...
        if ticks is not None:
            cb.set_ticks(ticks)
        cb.ax.tick_params(labelsize=8, length=2.2, pad=1.5)
        cb.outline.set_linewidth(0.5); cb.set_label(lab, fontsize=9.5, labelpad=3.0)
    ...
    field(ax[0, 2], om, "RdBu_r", -win, win, "(c) oracle", EM)
    im1 = field(ax[0, 3], cm, "RdBu_r", -win, win, "(d) curated", MAD)
    ...
    axt.set_xlabel("$x_2$ (transect at mid-domain)", fontsize=9.5)
    # The prior colours are the paper's convention and are named in the caption, so no legend.
    axt.set_ylabel("$u$", fontsize=9.5)
    ...
    axh.set_xlabel("oracle std", fontsize=9.5); axh.set_ylabel("curated std", fontsize=9.5)
    axh.yaxis.set_label_coords(-0.15, 0.5)
    axh.tick_params(labelsize=8); axh.set_aspect("equal")
    for sp in ("top", "right"):
