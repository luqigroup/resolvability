#!/usr/bin/env python
"""Generate the groundwater legacy-curation dataset: truths, observations, and the MAP archive.
...
Writes ``data/darcy/darcy_dataset.h5``: the oracle targets ``xi_true``, the curated targets
``xi_map``, the observations ``y_obs``, the train/eval ``split``, the operator's singular values ``sv``
...
def main():
    ...
    out = os.path.join(REPO, "data/darcy",
                       "darcy_dataset_smoke.h5" if args.smoke else "darcy_dataset.h5")
