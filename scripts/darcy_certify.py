...
standardized blind residual covers at the nominal level *whatever the prior reports* -- the prior
enters the band's width and never its validity. That is the paper's one constructive claim about a
trained prior, and this script is what tests it.

Two priors, so the claim can be seen doing work:
...
conditional spread s_rho, both read off a Gaussian fit to its own samples. The score is
...
m_rho +- q s_rho. Two quantities come out:
  ...
  coverage      of the band on further held-out truths. Must reach nominal for BOTH priors. This is
                the validity claim, and it is the half that does not care about the prior.
  width ratio   q / z_{1-alpha/2}: the certified half-width divided by the one the prior reports.
...
Reads the groundwater dataset, ``results/darcy_borehole.npz`` (the operator's exact-null
split), and the flow checkpoints ``data/checkpoints/darcy_flows/{prior}_s{seed}.pth``; all download
...
def main():
    ...
    ap.add_argument("--samples", type=int, default=8000, help="prior samples for the Gaussian fit")
    ...
    z = float(norm.ppf(1 - args.alpha / 2))                       # the width the prior reports
    ...
    res = {}
    print("\n  %-9s %-6s %-8s %-14s %s" % ("prior", "seed", "k", "coverage", "width / reported"))
    for tag in ("curated", "oracle"):
        for sd in args.seeds:
            X = prior_samples(tag, sd, args.samples)
    ...
    # The uncertified band is what the prior itself reports: m_rho +- z_{1-a/2} s_rho, the
    ...
    print("  %-9s %-8s %s" % ("prior", "band", "  ".join("%-7.2f" % L for L in lev)))
    for tag in ("curated", "oracle"):
        ...
        for nm, arr in (("reported", raw), ("certified", cert)):
