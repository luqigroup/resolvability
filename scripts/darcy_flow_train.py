...
Reads ``data/darcy/darcy_dataset.h5``; writes
``data/checkpoints/darcy_flow_{oracle,curated}_seed{seed}.pth``, each holding the best-validation
weights, the loss history, and the normalizer statistics. CPU only; both priors take a few minutes.
"""
...
def main():
    ...
    priors = [("oracle", xi_true), ("curated", xi_map)]
    if args.tag != "both":
        priors = [a for a in priors if a[0] == args.tag]
    for tag, X in priors:
