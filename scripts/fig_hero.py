...
def blind_coverage(Qb: np.ndarray) -> dict:
    ...
    The likelihood is flat on the blind subspace, so the posterior's blind marginal is the
    prior's: reading the prior's unconditional samples there IS reading the blind report.
    """
