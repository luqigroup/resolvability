...
PRIORS = (("oracle", "emcol"), ("curated", "madcol"))
...
def block(op_name: str, data: dict) -> list[str]:
    ...
    for sub in ("resolved", "blind"):
        ...
        for prior, colour in PRIORS:
            cells = [fmt(float(p["cov"][prior][i])) for i in idx]
            lead = op_name if (sub == "resolved" and prior == "oracle") else ""
            mid = sub if prior == "oracle" else ""
            out.append(f"{lead} & {mid} & \\textcolor{{{colour}}}{{{prior}}} & "
                       + " & ".join(cells) + r" \\")
    return out
