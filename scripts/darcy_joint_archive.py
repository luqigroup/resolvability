#!/usr/bin/env python
"""Legacy MAP archive curated JOINTLY against the head sensors and added boreholes.

This is the de-freezing corollary's archive. Joint curation de-freezes exactly the blind
directions the added boreholes resolve and nothing else, so the archive itself must be produced
under the STACKED operator -- reusing the sensors-only archive would silently invalidate the whole
comparison.

The comparison is PAIRED by construction: every row reuses its stored sensor observation
``y_obs[i]`` verbatim and only appends the borehole reading ``z_i = C xi_true_i + sigma_u eps``, so
the two archives differ by the boreholes and by nothing else -- not by a different noise draw, not
by a different observation.

Reads the groundwater dataset and ``results/darcy_borehole.npz`` (produced by
``scripts/darcy_build_borehole.py``); both download on first use. Writes
``results/darcy_joint_archive.npz`` with the jointly-curated coefficients, alongside the boreholes
they were curated against. Single core, no GPU; L-BFGS at ``maxiter=80`` because the augmented
problem needs more iterations than the sensors-only one (40 is binding once the boreholes are added).
...
def main():
    ...
    ap.add_argument("--sigma-u", type=float, default=0.04, help="borehole noise, KL units")
    ...
    # Seed the borehole noise from a SEPARATE stream. default_rng(0) is the stream the dataset
    # generator used, and its leading samples are xi_true itself -- reusing it would make the
    # "independent" borehole noise a recycled copy of the truth, which contradicts the hypothesis the
    # boreholes are supposed to satisfy even where they do not move the measured numbers.
    rng = np.random.default_rng([args.seed, 0xC0DE])
    z_all = simulate_boreholes(xi_true, C, args.sigma_u, rng)   # (n, 6) borehole readings
    ...
    for i in range(n):
        xi_joint[i] = map_reconstruct(
            ...
            maxiter=args.maxiter, borehole=(C, z_all[i], args.sigma_u))
