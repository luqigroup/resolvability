...
def main():
    ...
    # The two certified curves and the oracle's reported curve all land on the diagonal, where
    # they occluded one another (the oracle's certified series was invisible behind the others).
    # A small opposite dodge along the nominal axis, plus distinct marker sizes, keeps every series
    # readable without moving any plotted value off its level.
    for tag, col, dodge, ms in (("oracle", EM, -0.006, 6.4), ("curated", MAD, +0.006, 4.8)):
        ax[0].plot(lev + dodge, z[f"{tag}_cert"].mean(0), "s--", ms=ms, lw=1.0, color=col,
    ...
    ax[1].set_title("(b)  certified width / reported", fontsize=8.0, loc="left", pad=3.0)
