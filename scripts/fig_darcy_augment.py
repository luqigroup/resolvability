...
Both priors that were curated are drawn in the curated colour, solid for the sensors alone and open
for the joint curation, since the boreholes are the only difference between them.

Reads the groundwater dataset, ``results/darcy_borehole.npz``, and the flow checkpoints
``data/checkpoints/darcy_flows/{prior}_s10.pth`` for the oracle, curated and joint priors; all
...
@torch.no_grad()
def prior_samples(tag, seed=SEED, n=NDRAW):
...
def main():
    ...
    X = {t: prior_samples(t) for t in ("oracle", "curated", "joint")}
