...
Below the cutoff used here the curve is flat for a reason stronger than measurement: the blind basis
...
def main():
    ...
    ax.axvline(CUTOFF, color="0.55", lw=0.7, ls="-.", zorder=0)
    # Label placed ABOVE the axes rather than inside them: at its former y-span both curated curves
    # crossed it, so the annotation sat on top of the data it was pointing at.
    ax.annotate("cutoff used here", xy=(CUTOFF, 0.995), xycoords=("data", "axes fraction"),
                xytext=(3, -5), textcoords="offset points", fontsize=6.4, color="0.4",
                ha="left", va="top")
    ax.set_xscale("log")
    ...
    ax.set_title("(b)  and that the reading survives it", fontsize=8.0, loc="left", pad=12.0)
