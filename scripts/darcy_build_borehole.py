#!/usr/bin/env python
"""Build and cache the boreholes for the de-freezing repair.

Writes ``results/darcy_borehole.npz``: the selected borehole nodes, the borehole matrix ``C``, and
the de-frozen / control bases that split the operator's EXACT null. Everything here is computed
from the operator and the boreholes alone -- no truth, no archive, no data -- which is the point:
...
Run:  python scripts/darcy_build_borehole.py [--n-boreholes 6]
"""
...
from resolvability.groundwater.borehole import (build_borehole_matrix, borehole_subspaces,
                                                   select_borehole_nodes)
...
OUT = os.path.join(REPO, "results/darcy_borehole.npz")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--n-boreholes", type=int, default=6)
    ...
    B = sub["blind"]
    nodes = select_borehole_nodes(kl, B, n_boreholes=args.n_boreholes)
    C = build_borehole_matrix(kl, nodes)
    DF, CT = borehole_subspaces(C, B)
    sv = np.linalg.svd(C @ B, compute_uv=False)

    print(f"exact null {B.shape[1]}   de-frozen {DF.shape[1]}   control {CT.shape[1]}")
    print("boreholes:", [(int(i), int(j)) for i, j in nodes])
    print("borehole SNR at sigma_u=%g: %s" % (args.sigma_u,
