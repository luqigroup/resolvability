#!/usr/bin/env python
"""Run the shipped resolvability statement on the paper's two operators, and on the priors trained
for them.

The statement (``resolvability/statement.py``) has three outputs: the split itself, the
classification of reported functionals, and the share of a prior's reported variance
...
(``data/checkpoints/darcy_flows/{prior}_s{seed}.pth``), the seismic probe caches, and the cached
unconditional seismic samples; all download on first use. Writes
``results/run_statement.npz``. Memory: the seismic path holds one (160 x 65536) sample block
and a (65536 x 16) basis, a little over 100 MB. CPU, a few minutes.

Run:  python scripts/run_statement.py [--latex]
"""
...
OUT = os.path.join(REPO, "results/run_statement.npz")
SEEDS = (10, 11, 12)


@torch.no_grad()
def flow_samples(tag: str, seed: int, n: int = 8000) -> np.ndarray:
    """``n`` unconditional samples from one trained groundwater flow, in KL coordinates."""
...
def main():
    ...
    print("\n  share of each prior's reported variance lying on the blind subspace")
    print("  %-9s %-6s %-16s %s" % ("prior", "seed", "exact kernel", "near-null at noise"))
    for tag in ("curated", "oracle"):
        for sd in SEEDS:
            X = flow_samples(tag, sd)
    ...
    print("\n  share of each prior's reported variance certified blind (a LOWER bound)")
