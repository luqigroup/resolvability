...
def seismic():
    ...
    for key, kk in (("resolved", "res"), ("blind", "bl")):
        ...
        for prior in ("oracle", "curated"):
            P = [np.asarray(z[f"{prior}_{kk}_s{s}"], float) for s in range(nseed)]
            cov[prior, key] = np.array([[central_cov(p, Pt, a) for a in SEIS_LEVELS] for p in P])
            spread[prior, key] = np.array([p.var(0).sum() / vt for p in P])
    ...
    Tu = truth @ Qu.T
    for prior in ("oracle", "curated"):
        Pu = [np.concatenate([np.asarray(z[f"{prior}_res_s{s}"], float),
                              np.asarray(z[f"{prior}_bl_s{s}"], float)], 1) for s in range(nseed)]
        spread[prior, "random"] = np.array([
...
def groundwater():
    cov = {}
    for prior in ("oracle", "curated"):
        Z = [np.load(ensure(f"results/darcy_pcn_{prior}_s{s}.npz")) for s in DARCY_SEEDS]
        cov[prior, "resolved"] = np.array([z["resolved"] for z in Z])
        cov[prior, "blind"] = np.array([z["blind"] for z in Z])
