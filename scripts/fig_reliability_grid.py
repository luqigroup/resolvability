...
Reads ``results/seismic_prior_seed_coords.npz`` and ``results/darcy_pcn_{prior}_s{seed}.npz``.
...
# many samples the other row burned first, so reordering the rows would move every band.
...
def boot_ci_truths(pt: np.ndarray, per_seed: list[int], nboot: int = NBOOT_DARCY):
    ...
    if len(set(per_seed)) != 1:
        raise ValueError(
            f"seeds cover different truth counts {per_seed}; they are not interchangeable samples of "
...
def _panel(lev: np.ndarray, cov: dict, ci: dict, seeds: dict | None = None) -> dict:
    """Plot-ready panel, keyed by prior throughout: six same-typed positional arguments in
...
def seismic() -> dict:
    ...
    and a joint samples-by-truths bootstrap band on the seed-pooled samples.
    """
    ...
    for key, Q, kk in (("resolved", sub.Q_resolved, "res"), ("blind", sub.Q_blind, "bl")):
        Pt = truth @ Q.T
        cov, ci, seeds = {}, {}, {}
        for prior in ("oracle", "curated"):
            per = [np.asarray(z[f"{prior}_{kk}_s{si}"], float) for si in range(nseed)]   # (M, r) each
            curves = np.stack([agg_cov(P, Pt, SEIS_LEVELS) for P in per])              # (K, nL)
            cov[prior] = curves.mean(0)
            ci[prior] = agg_ci(np.concatenate(per, axis=0), Pt, SEIS_LEVELS)
            seeds[prior] = curves
...
def groundwater() -> dict:
    ...
    priors = {"oracle": load_prior("oracle"), "curated": load_prior("curated")}
    n_seeds = len(DARCY_SEEDS)
    lev = np.asarray(priors["oracle"][0]["levels"], dtype=float)
    ...
    cfg = {}
    for prior, S in priors.items():
        if len(S) != n_seeds:
            raise ValueError(f"{prior}: {len(S)} seeds, expected {n_seeds}")
        for i, s in enumerate(S):
            c = tuple(int(s[k]) for k in ("n_steps", "n_truths") if k in s.files)
            cfg.setdefault(c, []).append(f"{prior} s{DARCY_SEEDS[i]}")
            if not np.allclose(np.asarray(s["levels"], float), lev):
                raise ValueError(f"{prior} seed {DARCY_SEEDS[i]}: credible levels differ")
    if len(cfg) > 1:
        raise ValueError(
    ...
    for key in ("resolved", "blind"):
        ...
        for prior in ("oracle", "curated"):
            ...
            cov[prior], ci[prior], seeds[prior] = curves.mean(0), boot_ci_truths(pt, per_seed), curves
...
def main() -> None:
    ...
    for i, (rname, data) in enumerate(panels):
        for j, (ckey, ctitle, csub) in enumerate(COLS):
            ...
            for prior, col, mk in (("curated", MAD, "s"), ("oracle", EM, "o")):
                lo, hi = p["ci"][prior]
                ax.fill_between(lev, lo, hi, color=col, alpha=0.30, lw=0, zorder=3)
                ax.plot(lev, p["cov"][prior], marker=mk, ls="-", color=col, lw=1.9, ms=4.5,
                        zorder=5, label=prior)
                sd = p["seeds"][prior]
                if sd is not None:
