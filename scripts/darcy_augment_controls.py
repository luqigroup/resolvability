...
The de-freezing corollary predicts that joint curation moves the directions the boreholes resolve
and leaves the rest of the operator's exact null alone. The control directions do rise slightly, and
these two runs establish that the rise is the estimator's rather than information leaking in:

  placebo  a measurement carrying NO blind information at all -- the same borehole rows projected
           onto the resolved subspace, so ``C_placebo @ B = 0`` exactly -- moves the control FURTHER
           than the real boreholes do. An informative measurement cannot be beaten by an
           information-free one, so whatever moves the control is not the boreholes' content.

  sweep    varying the boreholes' noise level from sharp to nearly uninformative leaves the control
           flat while the de-frozen directions degrade smoothly. Leakage would scale with the
           measurement's strength; this does not.

Both rebuild the archive the same way ``darcy_joint_archive.py`` does, on a subsample by default,
since the point is a comparison against the sensors-only archive on the SAME rows rather than a
production archive. Single core, no GPU, and the arrays are all (n x 100) -- nothing here needs
more than a few MB.

Reads the groundwater dataset and ``results/darcy_borehole.npz`` (downloaded on first use).
...
from resolvability.groundwater.borehole import simulate_boreholes
...
DATA = "data/darcy/darcy_dataset.h5"
CHAN = "results/darcy_borehole.npz"
...
def main():
    ...
    ap.add_argument("--n", type=int, default=400, help="rows to curate per archive")
    ap.add_argument("--maxiter", type=int, default=80)
    ap.add_argument("--sigma-u", type=float, default=0.04, help="the borehole noise the paper uses")
    ...
    print("  %-22s %s" % ("archive", "  ".join("%-12s" % nm for nm, _ in sets)))
    report("sensors only", xi_map)
    real = curate(fwd, kl, sensors, y_obs, sigma_y, C, xi_true,
                  args.sigma_u, args.maxiter, args.seed, "real")
    report("+ real boreholes", real)

    if run_placebo:
        # Project every borehole row onto the resolved subspace: the placebo sees exactly what
        # the sensors already see, so it carries no blind information whatsoever.
        C_pl = C - (C @ B) @ B.T
        ...
        report("+ placebo", X)
        print("  -> placebo moves the control %s than the real boreholes"
              % ("MORE" if rows["+ placebo"][1] > rows["+ real boreholes"][1] else "LESS"))

    if run_sweep:
        print("\n  borehole-strength sweep (control must stay flat; de-frozen must degrade)")
        for su in args.sweep_sigma:
            X = curate(fwd, kl, sensors, y_obs, sigma_y, C, xi_true,
                       su, args.maxiter, args.seed, f"sigma_u={su:g}")
            report("+ boreholes, sigma_u=%g" % su, X)
