"""The two seismic priors: architecture, checkpoint lookup, evaluation images, sampling.
...
``curated``  the legacy least-squares migration archive (``lsrtm5``) -- the prior the paper studies.

Three training seeds per prior are shipped, as ``data/checkpoints/seismic_prior_<tag>_seed<i>.pth``.
...
BASE = 64                    # UNet width of both priors
N_SEEDS = 3                  # training seeds shipped per prior
...
def find_all_ckpts(tag: str, n_seeds: int = N_SEEDS) -> list[tuple[str, int]]:
    ...
    Returns ``(path, base)`` pairs ordered by the seed index in the filename, so the prior's seed
