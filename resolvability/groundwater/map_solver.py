...
def _neg_log_post(xi, fwd, kl, sensors, y_obs, sigma_y, borehole=None):
    """Return ``(J, grad_xi)`` for the whitened-KL negative log-posterior.

    ``borehole`` : optional ``(C, z, sigma_u)`` added measurement, independent of the sensors given
    the field -- the de-freezing corollary's boreholes. ``C`` is linear in the coefficients (a
    borehole reads ``u`` directly), so it contributes ``||C xi - z||^2 / (2 sigma_u^2)`` and its
...
def map_reconstruct(fwd, kl, sensors, y_obs, sigma_y, *, xi0=None, maxiter: int = 200,
    ...
    Args:
        ...
        borehole: optional ``(C, z, sigma_u)`` added measurement (the de-freezing
            corollary's boreholes). Curating JOINTLY means the archive itself is produced under the stacked
            operator, so this must be passed when building the joint archive -- reusing a
            sensors-only archive silently invalidates the comparison. Note the augmented problem
            needs a larger ``maxiter`` than the sensors-only one.
    ...
    def fun(xi):
        return _neg_log_post(xi, fwd, kl, sensors, y_obs, sigma_y, borehole=borehole)
