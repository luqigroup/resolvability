...
class DarcySetup:
    ...
    def eval_truths(self, n_truths: int):
        """First ``n_truths`` held-out observations: ``(y_obs, xi_true)``, in dataset order."""
...
def load_flow(prior: str, seed: int = 0, device: str | torch.device = "cpu"):
    """Load a trained HINT prior. Returns ``(flow, normalizer)``.

    Args:
        prior: ``"oracle"`` (trained on truths) or ``"curated"`` (trained on the MAP archive).
        seed: training seed, 0-2.
        device: torch device; the d=100 flow runs comfortably on CPU.
    """
    ck = torch.load(ensure(f"data/checkpoints/darcy_flow_{prior}_seed{seed}.pth"),
...
def pcn_posterior(flow, norm, st: DarcySetup, y_obs, *, n_steps, n_burn, beta, thin,
    ...
    Args:
        ...
        y_obs: (nt, n_sensors) observations, one chain per row.
