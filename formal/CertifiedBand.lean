...
> *Let `x₁, …, x_k` be references exchangeable with the truth reported on and held out of the
...
* **Conditional validity** (`certified_blind_band_conditional`): the secondary claim.  The
  ...
  observed datum `y` the conditional law of `(θ⋆, x₁, …, x_k)` given `y` is still
...
1. `hexch` — exchangeability of the references with the truth reported on: the proposition's own
...
3. In `certified_blind_band_conditional` only: the conditional exchangeability/distinctness
   given the observed datum — the formal residue of the paper's resolved–blind independence.
...
`m_ρ(x_R), s_ρ` — the prior's; arbitrary here, hence "whatever the prior reports"), and any
0-indexed order-statistic position `j` with `(k+1)(1−α) ≤ j+1`: the interval band
`[m − η₍ⱼ₊₁₎·s, m + η₍ⱼ₊₁₎·s]` built from the calibration scores covers the truth reported on,
...
/-- **`p:certify`, verbatim endpoint.**  References exchangeable with the truth reported on,
...
(`band_eq_Icc`) — covers the truth reported on, `θ⋆ = x 0`, with probability at least `1 − α`,
...
under resolved–blind independence the guarantee holds conditionally on the observed datum.
The independence enters ONLY through the NAMED HYPOTHESES `hexch`/`hdist`: for a.e. observed
