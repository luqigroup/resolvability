...
* `curatedPrior_blind_freeze` — reuses Theorem 1's Part A (`ArchiveLaw.pLaw_partA`,
...
lemma measurable_emReweight {𝓧 : Type*} [MeasurableSpace 𝓧]
    ...
    Measurable (ArchiveLaw.emReweight κ μ ν) := by
...
lemma emReweight_factors {𝓧 : Type*} [MeasurableSpace 𝓧] [Nonempty B]
    ...
    (hκ : ∀ x x' : R × B, x.1 = x'.1 → κ x = κ x')
    (h_ac : κ ∘ₘ ν ≪ κ ∘ₘ μ) :
    ∃ f : R → ℝ≥0∞, Measurable f ∧ ArchiveLaw.emReweight κ μ ν = fun p => f p.1 := by
  classical
  refine ⟨fun r => ArchiveLaw.emReweight κ μ ν (r, Classical.arbitrary B),
...
`curatedPrior = reg.withDensity (emReweight)` (`ArchiveLaw.pLaw_partA`).  With the regularizer
...
theorem curatedPrior_blind_freeze
    ...
    ∃ f : R → ℝ≥0∞, Measurable f ∧
      ArchiveLaw.curatedPrior κ (νR ⊗ₘ κreg) ν = (νR.withDensity f) ⊗ₘ κreg := by
  obtain ⟨f, hf, hfeq⟩ := emReweight_factors κ (νR ⊗ₘ κreg) ν hκ h_ac
  exact ⟨f, hf, by rw [ArchiveLaw.pLaw_partA κ (νR ⊗ₘ κreg) ν h_dom h_ac, hfeq,
