...
Reusing Theorem 1's Part A (`ArchiveLaw.pLaw_partA`, `curatedPrior = reg.withDensity
...
step). -/
private lemma measurable_emReweight : Measurable (ArchiveLaw.emReweight κ μ ν) := by
...
theorem fixed_point_iff_reweight_ae_one
    ...
    ArchiveLaw.curatedPrior κ μ ν = μ ↔ ArchiveLaw.emReweight κ μ ν =ᵐ[μ] 1 := by
  rw [ArchiveLaw.pLaw_partA κ μ ν h_dom h_ac]
  constructor
  · intro h
    have hh : μ.withDensity (ArchiveLaw.emReweight κ μ ν) = μ.withDensity 1 := by
...
theorem reweight_ae_one_of_data_marginal_eq
    ...
    ArchiveLaw.emReweight κ μ ν =ᵐ[μ] 1 := by
...
theorem data_marginal_eq_imp_fixed_point
    (h_dom : ∀ᵐ x ∂μ, κ x ≪ κ ∘ₘ μ) (h_eq : κ ∘ₘ ν = κ ∘ₘ μ) :
    ArchiveLaw.curatedPrior κ μ ν = μ :=
...
theorem fixed_point_imp_data_marginal_eq
    ...
    (hfix : ArchiveLaw.curatedPrior κ μ ν = μ) :
    κ ∘ₘ ν = κ ∘ₘ μ := by
  ...
  have hrwμ : ArchiveLaw.emReweight κ μ ν =ᵐ[μ] 1 :=
    (fixed_point_iff_reweight_ae_one κ μ ν h_dom h_ac).mp hfix
  have hrwν : ArchiveLaw.emReweight κ μ ν =ᵐ[ν] 1 := hνμ.ae_le hrwμ
  ...
  have hchi_enn : ∫⁻ y, ((κ ∘ₘ ν).rnDeriv (κ ∘ₘ μ) y) ^ 2 ∂(κ ∘ₘ μ) = 1 := by
    calc ∫⁻ y, ((κ ∘ₘ ν).rnDeriv (κ ∘ₘ μ) y) ^ 2 ∂(κ ∘ₘ μ)
      ...
      _ = ∫⁻ x, ArchiveLaw.emReweight κ μ ν x ∂ν := by
          rw [← lintegral_lintegral_swap (μ := ν) (ν := κ ∘ₘ ν)
            (f := κ.rnDeriv (Kernel.const Ω (κ ∘ₘ μ))) hDmeas.aemeasurable]
          rfl
      _ = 1 := by
          have h1 : ArchiveLaw.emReweight κ μ ν =ᵐ[ν] fun _ => (1 : ℝ≥0∞) := by
