...
posterior samples `θᵢ = f i.succ`, and ranks the truth among the samples.  The paper's remark
...
truth and the posterior samples are exchangeable" — the exchangeability itself is a *modelling*
...
/-- The SBC rank of the test truth `θ = f 0` among the `N` posterior samples `θᵢ = f i.succ`:
`R = #{i : θᵢ < θ}` (the paper's rank statistic in `r:selfcheck`). -/
noncomputable def sbcRank (f : Fin (N + 1) → ℝ) : ℕ :=
  #{i : Fin N | f i.succ < f 0}

/-- The SBC rank **is** the position statistic of coordinate `0`: counting the samples below the
...
`μ {R = k} = (N+1)⁻¹` for every `k ≤ N`, whatever the prior that generated the samples. -/
...
posterior samples is uniform on `{0, …, N}`. -/
...
/-- **The rank law does not depend on which exchangeable prior generated the samples.**  Any two
