...
/-! ## Identical trained prior (reusing `ArchiveLaw.curatedPrior`) -/
...
theorem curatedPrior_depends_only_on_data_marginal
    ...
    ArchiveLaw.curatedPrior κ μ ν = ArchiveLaw.curatedPrior κ μ ν' := by
  unfold ArchiveLaw.curatedPrior
  rw [h]
...
theorem nonident_curatedPrior
    ...
    (h : ν.map Prod.fst = ν'.map Prod.fst) :
    ArchiveLaw.curatedPrior κ μ ν = ArchiveLaw.curatedPrior κ μ ν' :=
