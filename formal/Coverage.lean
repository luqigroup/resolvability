...
against the true law `𝒩(m_⋆, s_⋆²)` the reported central interval `m_reg ± z·s_reg`
...
/-- **`p:cover` — the closed-form coverage** (eq:coverage-gen). The reported interval
...
coverage `C(0, z s_reg/s_⋆)` strictly below the nominal `C(0, z) = 2Φ(z) − 1 = 1 − α`: the reported
...
`r = z h c_K / s_⋆ → 0`), the reported coverage `C(0, r) = 2Φ(r) − 1 → 0`.  A generative estimator
...
conditional mean about the reported center leaves the covered mass unchanged, because the
...
there — the fiber-conditional coverage is strictly below the reported level `C(0, z) = 1 − α`.
...
* nominal `= 2Φ(z) − 1 = 1 − α` — **(a)**: `C(0, z) = 2Φ(z) − 1` (`C_zero`) is the coverage of a
  correctly specified interval, i.e. the reported level `1 − α` by the definition of `z`.
...
* **The smoothing floor** (`r:map-lg`, `tendsto_C_zero_atZero`): `C(0, r) = 2Φ(r) − 1 → 0` as the
  width ratio `r → 0` (from `Φ` continuous, `Φ(0) = 1/2`) — the vanishing reported coverage a
