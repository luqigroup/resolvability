...
## `c:augment` (general half) — an added measurement de-freezes exactly the directions it resolves

Let `A` be the operator with blind subspace `ker A`, and let a new measurement resolve a subspace
`𝔅₁ ⊆ ker A` — i.e. the measurement `C` (the paper's `B₁ᵀ`) has `ker C = 𝔅₁ᗮ`.  Curating jointly against
...
`ker A ⊓ ker C = ker A ⊓ 𝔅₁ᗮ` when the new measurement resolves exactly `𝔅₁` (`ker C = 𝔅₁ᗮ`). -/
...
* `ker C = 𝔅₁ᗮ` (the new measurement resolves exactly `𝔅₁`) — **(a)**: the paper's `B₁` has orthonormal
  columns spanning `𝔅₁`, so its adjoint `B₁ᵀ` has kernel `(range B₁)ᗮ = 𝔅₁ᗮ`.  Taken here as the
  measurement's defining property (the orthonormality/adjoint step is the standard `ker Bᵀ = (range B)ᗮ`).
