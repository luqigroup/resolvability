...
theorem curatedPrior_condKernel_freeze
    ...
    (h_ac : κ ∘ₘ ν ≪ κ ∘ₘ ρ) :
    ArchiveLaw.curatedPrior κ ρ ν
        = (ArchiveLaw.curatedPrior κ ρ ν).fst ⊗ₘ ρ.condKernel ∧
      ∀ᵐ r ∂(ArchiveLaw.curatedPrior κ ρ ν).fst,
        (ArchiveLaw.curatedPrior κ ρ ν).condKernel r = ρ.condKernel r := by
  obtain ⟨f, hf, hfeq⟩ := BlindFreeze.emReweight_factors κ ρ ν hκ h_ac
  have hq : ArchiveLaw.curatedPrior κ ρ ν = ρ.withDensity fun p => f p.1 := by
    rw [ArchiveLaw.pLaw_partA κ ρ ν h_dom h_ac, hfeq]
  have hcomp : ArchiveLaw.curatedPrior κ ρ ν
      = (ArchiveLaw.curatedPrior κ ρ ν).fst ⊗ₘ ρ.condKernel := by
    rw [hq]
...
lemma measurable_emReweight_general : Measurable (ArchiveLaw.emReweight κ μ ν) := by
  unfold ArchiveLaw.emReweight
...
lemma emReweight_congr (h_ac : κ ∘ₘ ν ≪ κ ∘ₘ μ) {x x' : Ω} (hxx : κ x = κ x') :
    ArchiveLaw.emReweight κ μ ν x = ArchiveLaw.emReweight κ μ ν x' := by
  unfold ArchiveLaw.emReweight
...
theorem hilbert_freeze_assembled
    ...
    ∃ κc : Kernel (↥(A.ker)ᗮ) (↥A.ker), IsMarkovKernel κc ∧
      ...
      (ArchiveLaw.curatedPrior κ reg ν).map (resolvedBlindSplit A).symm
        = ((ArchiveLaw.curatedPrior κ reg ν).map (resolvedBlindSplit A).symm).fst ⊗ₘ κc ∧
      ∀ κ' : Kernel (↥(A.ker)ᗮ) (↥A.ker), IsFiniteKernel κ' →
        (ArchiveLaw.curatedPrior κ reg ν).map (resolvedBlindSplit A).symm
          = ((ArchiveLaw.curatedPrior κ reg ν).map (resolvedBlindSplit A).symm).fst ⊗ₘ κ' →
        κ' =ᵐ[((ArchiveLaw.curatedPrior κ reg ν).map (resolvedBlindSplit A).symm).fst] κc := by
  ...
  set q : Measure X := ArchiveLaw.curatedPrior κ reg ν with hqdef
  ...
  have hgmeas : Measurable (ArchiveLaw.emReweight κ reg ν) :=
    measurable_emReweight_general κ reg ν
  have hfmeas : Measurable fun r : ↥(A.ker)ᗮ =>
      ArchiveLaw.emReweight κ reg ν (e (r, 0)) :=
    hgmeas.comp (e.measurable.comp (measurable_id.prodMk measurable_const))
  have hfactor : ∀ x : X, ArchiveLaw.emReweight κ reg ν x
      = ArchiveLaw.emReweight κ reg ν (e (((e.symm x).1), 0)) := by
    intro x
  ...
  have hwmeas : Measurable fun p : ↥(A.ker)ᗮ × ↥A.ker =>
      ArchiveLaw.emReweight κ reg ν (e (p.1, 0)) :=
    hfmeas.comp measurable_fst
  have hmap : q.map e.symm = (reg.map e.symm).withDensity
      (fun p => ArchiveLaw.emReweight κ reg ν (e (p.1, 0))) := by
    have hq' : q = reg.withDensity fun x =>
        (fun p : ↥(A.ker)ᗮ × ↥A.ker =>
          ArchiveLaw.emReweight κ reg ν (e (p.1, 0))) (e.symm x) := by
      rw [hqdef, ArchiveLaw.pLaw_partA κ reg ν h_dom h_ac]
      congr 1
      funext x
      exact hfactor x
    rw [hq']
