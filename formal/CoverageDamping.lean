...
> "a Gaussian regularizer with curvature `μ_v` along a blind direction reports a conditional
...
* `condVar P v := ⟪v, (P₂₂)⁻¹ v⟫`, `spread P v := √(condVar P v)` — the **reported
...
`condVar` *defines* the reported fiber-conditional variance as the `v`-form of the inverse
...
* `inv_curvature_le_condVar` / `inv_sqrt_curvature_le_spread` — `condVar ≥ μ_v⁻¹`, so the
  reported conditional spread is at least `μ_v^{−1/2}`;
...
* `spread_antitone_damping`, `coverage_antitone_damping` — hence the reported spread and the
...
modeling identification (reported fiber conditional = inverse blind-block precision, its
...
Completing the square: for `M ≻ 0` and unit `v`, the gap between the reported variance
...
/-- **Step 1, the inequality**: the reported variance along `v` is at least the curvature's
...
the reported variance: `⟪v, Mv⟫⁻¹ < ⟪v, M⁻¹v⟫`. -/
...
/-! ## Step 2 — the blind block: curvature `μ_v`, reported conditional variance and spread -/
...
/-- The **reported fiber-conditional variance** along `v` given the resolved coordinates: the
...
/-- The **reported conditional spread** `s_ρ` along `v`. -/
...
/-- **The spread lower bound at the blind block**: the reported fiber-conditional variance is at
...
omit [DecidableEq R] in
/-- **"…reports a conditional spread of at least `μ_v^{−1/2}`"**: `μ_v^{−1/2} ≤ s_ρ`. -/
...
reported fiber-conditional variance built from the *precision* `Σ⁻¹` is exactly the `v`-form of
...
/-- **The plug-in core of `eq:covmu`**: any reported spread `s_ρ ≥ μ^{-1/2}` yields aligned
...
blind direction `v` reports the central band `m ± z·s_ρ`; against a truth of blind marginal
...
to the precision can only shrink the reported fiber-conditional variance:
...
omit [Fintype R] [DecidableEq R] in
/-- Hence the reported conditional spread is nonincreasing in the damping. -/
