...
/-!
# Observations and references compared, and why only one stays finite (Remark `r:exchange`)

Machine-checked assembly of the paper's Remark *"Observations and references compared, and
why only one stays finite"* (`r:exchange` of the paper): the same rank-one blind-spread
misreport `Σ⋆ ↦ Σ⋆ + w·vvᵀ` can be pursued in observations or in reference truths, and the two
requirements behave oppositely.  Almost everything here is REUSED from already-checked modules —
...
covariance), and `AuditCount.lean` (the audit's χ² laws, power and level) — and this module
...
**(2) The observation count diverges.**  `blindGap_le_noise_floor` proves
...
`n < 2·(γ/(w‖Av‖²))²` — a guaranteed-indistinguishable observation budget that `observation_count_diverges`
...
summed error `≥ 1`.  `observation_count_mean` is the mean-misreport analogue (`n < γ/(w²‖Av‖²)`,
from `MeanKL.lean`): both moments' observation counts diverge.

**(3) The reference count does not involve `A`.**  `surveyedAuditNullLaw` / `surveyedAuditAltLaw`
/ `surveyedAuditPower` are the audit's null law, alternative law and rejection probability
written WITH a forward operator in scope — and `audit_count_operator_free` proves (by `rfl`,
...
operator-free `referenceAuditPower k b r q = χ²_{kb}(Ioi (q/r²))` (reusing `AuditCount.audit_power`),
and `surveyedAuditLevel` re-exports the level bound.  `reference_count_pipeline` runs the audit
...
**(4) CAPSTONE `observations_vs_references`.**  One theorem packaging (2)+(3): for the physical record laws
...
`n` observations separates the two truths (summed error `> 1/2`); (ii) on a genuine kernel
...
anywhere in the formula.  The two do not trade off at a fixed rate.
...
(the latter deliberately absent from `AuditCount.lean` as well).
...
namespace ObservationsVsReferences
...
/-! ### (2b) The observation count diverges -/

/-- **The observation crossover diverges as the direction goes blind:** for a fixed misreport `w > 0`
and noise floor `γ > 0`, the guaranteed-indistinguishable observation budget `2·(γ/(w·‖Av‖²))²` of
the checked spread crossover tends to `∞` as the illumination `‖Av‖² → 0⁺`.  The two requirements
do not trade off at a fixed rate. -/
theorem observation_count_diverges {w γ : ℝ} (hw : 0 < w) (hγ : 0 < γ) :
...
/-- **The mean-misreport observation count also diverges** (the supplementary moment, via
`MeanKL.lean`'s checked `n < 1/β` crossover): with the noise floor `γ`, whenever
`n·(w²‖Av‖²/γ) < 1` no test on `n` observations separates a `w·v` blind-mean misreport — a budget
diverging as the inverse FIRST power of the illumination. -/
theorem observation_count_mean {d m : ℕ}
...
/-! ### (3) The reference count does not involve the operator -/

/-- The audit's null law, written WITH a forward operator in scope (as a field record would carry
one).  The definition provably never reads it: see `audit_count_operator_free`. -/
...
/-- **The audit's rejection probability as a function of `(k, b, r, q)` alone** — no operator argument
exists to pass. -/
noncomputable def referenceAuditPower (k b : ℕ) (r q : ℝ) : ℝ≥0∞ :=
  AuditCount.chiSq (k * b) (Set.Ioi (q / r ^ 2))
...
law and rejection probability coincide, definitionally (`rfl`): the reference-truth audit is the same
whether the direction is faintly illuminated or exactly blind, because the operator never
enters. -/
theorem audit_count_operator_free {d₁ m₁ d₂ m₂ : ℕ}
...
`AuditCount.auditStat_null_law`). -/
theorem surveyedAuditNullLaw_eq_chiSq {d m : ℕ}
    (Aop : EuclideanSpace ℝ (Fin d) →L[ℝ] E m) (k b : ℕ) :
    surveyedAuditNullLaw Aop k b = AuditCount.chiSq (k * b) :=
  AuditCount.auditStat_null_law k b

/-- **The power factors through the operator-free expression:**
`surveyedAuditPower A k b r q = referenceAuditPower k b r q = χ²_{kb}(Ioi (q/r²))` (re-export of
`AuditCount.audit_power`). -/
theorem surveyedAuditPower_factors {d m : ℕ}
    (Aop : EuclideanSpace ℝ (Fin d) →L[ℝ] E m) (k b : ℕ) {r : ℝ} (hr : 0 < r) (q : ℝ) :
    surveyedAuditPower Aop k b r q = referenceAuditPower k b r q :=
  AuditCount.audit_power k b hr q

/-- The audit's level over the composite null, whatever the operator (re-export of
`AuditCount.audit_level`). -/
theorem surveyedAuditLevel {d m : ℕ}
    (Aop : EuclideanSpace ℝ (Fin d) →L[ℝ] E m) (k b : ℕ) {r q : ℝ}
    (hr : 0 < r) (hr1 : r ≤ 1) (hq : 0 ≤ q) :
    surveyedAuditPower Aop k b r q ≤ AuditCount.chiSq (k * b) (Set.Ioi q) :=
  AuditCount.audit_level k b hr hr1 hq

/-- **The reference audit at the physical witness** (one-dimensional blind fiber, the rank-one
...
theorem reference_count_pipeline (k : ℕ) (mB : EuclideanSpace ℝ (Fin 1)) {w s : ℝ}
    (hw : 0 ≤ w) (hs : 0 < s) (q : ℝ) :
    ((Measure.pi fun _ : Fin k =>
        multivariateGaussian mB ((s ^ 2 + w) • (1 : Matrix (Fin 1) (Fin 1) ℝ))).map
        (fun x => AuditCount.auditStat fun i =>
          toEuclideanCLM (𝕜 := ℝ) (s⁻¹ • (1 : Matrix (Fin 1) (Fin 1) ℝ)) (x i - mB)))
        (Set.Ioi q)
      = AuditCount.chiSq k (Set.Ioi (q * s ^ 2 / (s ^ 2 + w))) := by
  ...
  have hsqrtS : CFC.sqrt (s ^ 2 • (1 : Matrix (Fin 1) (Fin 1) ℝ)) = s • 1 := by
    rw [AuditCount.sqrt_sq_smul hs.le Matrix.PosSemidef.one, hsqrt1]
  ...
  rw [hcov, AuditCount.audit_power_pipeline k 1 mB (s ^ 2 • (1 : Matrix (Fin 1) (Fin 1) ℝ))
    (s⁻¹ • (1 : Matrix (Fin 1) (Fin 1) ℝ)) hS hW hrpos q, mul_one, harg]

/-! ### (4) CAPSTONE: the observation count diverges, the reference count does not -/

/-- **Observations and references compared (Remark `r:exchange`), assembled.**
...
(i) **observations below the diverging crossover discriminate nothing**: whenever
`n·(w‖Av‖²/γ)² < 2` — in verbatim `n⋆` form, `n < 2·(γ/(w‖Av‖²))²`, which diverges as the
inverse square of the illumination — every test on `n` iid observations has summed type-I + type-II
error strictly above `1/2` (reusing `SpreadKL.model_spread_kl_crossover_closed`);

(ii) **on a genuine kernel no number of observations suffices**: at `Av = 0` the two record laws are EQUAL
(the checked per-survey KL is `0`; converse Gibbs), so at EVERY observation count every test has
summed error at least `1`;

(iii) **the reference count is fixed and operator-free**: the `k`-reference audit at the same
...
exactly blind. -/
theorem observations_vs_references {d m : ℕ}
  ...
  -- the observation clause, proved once
  ...
  · -- the reference audit at the same witness, operator-free
    intro k q
    have hs2 : (s : ℝ) ^ 2 ≠ 0 := pow_ne_zero 2 hs.ne'
    have h1 : (s ^ 2 + w) / s ^ 2 = 1 + w / s ^ 2 := by
      rw [add_div, div_self hs2]
    ...
    calc surveyedAuditPower Aop k 1 (spreadRatio w s) q
        = AuditCount.chiSq (k * 1) (Set.Ioi (q / spreadRatio w s ^ 2)) :=
          AuditCount.audit_power k 1 (spreadRatio_pos hw s) q
      _ = AuditCount.chiSq k (Set.Ioi (q * s ^ 2 / (s ^ 2 + w))) := by
          rw [mul_one, harg]

end ObservationsVsReferences

-- #print axioms audit (only propext / Classical.choice / Quot.sound expected)
#print axioms ObservationsVsReferences.spreadRatio_sq
#print axioms ObservationsVsReferences.one_le_spreadRatio
#print axioms ObservationsVsReferences.spreadRatio_pos
#print axioms ObservationsVsReferences.spreadRatio_sq_mul
#print axioms ObservationsVsReferences.spreadRatio_witness
#print axioms ObservationsVsReferences.spreadRatio_strictMonoOn
#print axioms ObservationsVsReferences.spreadRatio_bijOn
#print axioms ObservationsVsReferences.exists_noise_floor
#print axioms ObservationsVsReferences.blindGap_le_noise_floor
#print axioms ObservationsVsReferences.observation_count_diverges
#print axioms ObservationsVsReferences.observation_count_mean
#print axioms ObservationsVsReferences.audit_count_operator_free
#print axioms ObservationsVsReferences.surveyedAuditNullLaw_eq_chiSq
#print axioms ObservationsVsReferences.surveyedAuditPower_factors
#print axioms ObservationsVsReferences.surveyedAuditLevel
#print axioms ObservationsVsReferences.reference_count_pipeline
#print axioms ObservationsVsReferences.observations_vs_references
