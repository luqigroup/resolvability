...
`eq:pooled` were cut in a page-limit trim and appear in no section of the current paper. Their Lean proofs
...
|---|---|
| `p:audit`'s closing asymptotic `k ≈ 1+(z_{1−α}+z_{1−β})²/(2b log² r)`, and the rank-one search-penalty aside | paper only, **deliberate** — asymptotic approximations; the exact power and level content behind them is checked in `AuditCount.lean` / `ObservationsVsReferences.lean` |
...
Wave 5 modules: `ObservationsVsReferences.lean` (observations and references at one blind direction — the
observation count from the checked `SpreadKL`/`MeanKL` crossovers, the reference count from
`AuditCount`, and the divergence of the first as `‖Av‖² → 0`), `MapNearNullCoverage.lean` ((ii)'s coverage tail, with the
...
* `p:audit`'s closing **asymptotic** `k ≈ 1+(z_{1−α}+z_{1−β})²/(2b log² r)` and the discussion's
  ...
  the exact power/level content behind them is checked in `AuditCount.lean`/`ObservationsVsReferences.lean`.
